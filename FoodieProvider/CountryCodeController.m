...
- (void)viewDidLoad {
    ...
    [self parseJSON];
    _countrySsearchBar.delegate = self;
}
...
- (void)searchBar:(UISearchBar *)searchBar textDidChange:(NSString *)searchText
{
    if (searchText.length==0) {
        ...
        [self.countrysTableView reloadData];
    }
}
- (void)searchBarSearchButtonClicked:(UISearchBar *)searchBar{
    [self.countrysTableView resignFirstResponder];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if(!isFiltered){
        cell.countrysNameLbl.text = [[countriesList objectAtIndex:indexPath.row] valueForKey:kCountryName];
        cell.countrysCodeLbl.text = [[countriesList objectAtIndex:indexPath.row] valueForKey:kCountryCallingCode];
        cell.flagsImageView.image = image;
    }
    else{
        NSString *countryCode = [NSString stringWithFormat:@"%@", [[filteredstring objectAtIndex:indexPath.row] valueForKey:kCountryCode]];
        NSString *imagePath = [NSString stringWithFormat:@"CountryPicker.bundle/%@", countryCode];
        UIImage *image = [UIImage imageNamed:imagePath];
        cell.countrysNameLbl.text = [[filteredstring objectAtIndex:indexPath.row] valueForKey:kCountryName];
        cell.countrysCodeLbl.text = [[filteredstring objectAtIndex:indexPath.row] valueForKey:kCountryCallingCode];
        cell.flagsImageView.image = image;
    }
