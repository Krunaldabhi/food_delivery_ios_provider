...
-(void)getDataFromPath:(NSString *)path withParamData:(NSDictionary *)dictParam withBlock:(RequestCompletionBlock)block
{
    ...
    {
        [manager DELETE:strURL parameters:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject)
         {
             [self stop];
             if(dataBlock){
    ...
    {
        [manager PATCH:strURL parameters:dictParam success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject)
         {
             [self stop];
    ...
    }else if ([self.strReqMethod isEqualToString:PUT_METHOD]) {
        ...
        [manager PUT:strURL parameters:dictParam success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
            ...
            {
                dataBlock(nil,serializedData,@"4");
            }
        }];
    }
        ...
        [manager POST:url parameters:dictParam constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
           // [formData appendPartWithFileData:imageToUpload name:PICTURE fileName:@"files" mimeType:@"image/png"];
            ...
            for(UIImage *eachImage in images)
            {
                ...
                [formData appendPartWithFileData:imageData name:[NSString stringWithFormat:@"picture"] fileName:[NSString stringWithFormat:@"photo%d.png",i ] mimeType:@"image/png"];
                i++;
            }
...
-(void)getDataFromPath:(NSString *)path withParamDataImages:(NSDictionary *)dictParam andImagesArray:(NSArray *)images withBlock:(RequestCompletionBlock)block{
    ...
    if ([images count])
    {
        ...
        [manager POST:url parameters:dictParam constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
            ...
            {
                NSData *imageData = UIImagePNGRepresentation(eachImage);
                [formData appendPartWithFileData:imageData name:[NSString stringWithFormat:@"image[%d]",i ] fileName:[NSString stringWithFormat:@"imageFiles[%d]",i ] mimeType:@"image/png"];
                i++;
            }
...
-(void)start
{
    if ([self.loaderRequestStr isEqualToString:@"Hide"]) {
        loader = nil;
    }
    else{
